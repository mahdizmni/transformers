...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, num_examples=2):
    ...
    console_width = 80
    with torch.no_grad():                   # is this neccessary? model.eval should handle it?
...
def get_ds(config):
    ...
    print(f'Max len of source sent: {max_len_src}')
    print(f'Max len of target sent: {max_len_tgt}')
...
def train_model(config):
    ...
    # Tensorboard (visual loss)
    writer = SummaryWriter(config['experiment_name'])
    
    optimizer = torch.optim.Adam(model.parameters(), lr=config['lr'], eps=1e-9)

    # restore state of model / optimizer if crashed during training
    initial_epoch = 0
    global_step = 0
    if config['preload']:
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # Backprop the loss (compute gradient)
            loss.backward()
        ...
        # save the model at then end of every epoch
